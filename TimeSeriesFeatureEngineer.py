...
def create_average_feature(input_df,value_column,new_column_name,window,step):
    ...
    """
    df=input_df.copy()
    df.loc[df.index[(np.arange(len(df))+1)%step==0],new_column_name]=df[value_column].rolling(window=window).mean()
    return df
...
class TimeSeries:
    ...
    def get_k_train_1_test_data_for_boosting(self,target_name,feture_names,train_set_number=1,test_size=.3):
        data=self.data.dropna()
        y = np.array(list(data[target_name]))
        features=tuple([list(data[name]) for name in feture_names])
        x = np.array(list(zip(*features)))
        ...
        for i in range(train_set_number):
            if i+1==train_set_number:
                ...
                train_dict[i]=[x_train[i * each_train_size:(i+1)*each_train_size, :], y_train[i*each_train_size:(i+1)*each_train_size, ]]
