...
'''
def remove_test_project_from_sow(projectSowData,projectID_column='ProjectID',
                                 StatmentOfWork_column='StatementOfWork'):
    df=projectSowData.copy()
    removed_dict=dict()
    pattern = re.compile(r'^test|\stest$|\stesting\s|\stesting|testing\s|example|template',re.IGNORECASE)
...
def remove_example_project_from_df(projectSowData,projectID_column='ProjectID',target_remove_column='StatementOfWork'):
    df=projectSowData.copy()
    removed_dict=dict()
...
'''
def remove_projects_by_sow_length(projectSowData, lower_bound=None, upper_bound=None,projectID_column='ProjectID',
                                 StatmentOfWork_column='StatementOfWork'):
...
'''
def remove_outlier_by_cc_freq(projectccdf,minimal_freq=1,projectID_column='ProjectID',
...
def remove_data_points_for_CC_level(projectSowData,keepiDigit,remove_string='00',projectID_column='ProjectID'):
    '''
    This function expect no nan in the inout datafram, so replace nan with empty string
...
def text_tokens(text,lower_bound_percentage=0,higher_bound_percentage=1,minimal_word_length=0,
    ...
    tokens=tokens[int(howmany_tokens*lower_bound_percentage):int(ceil(howmany_tokens*higher_bound_percentage))]
    ...
    if lemmatize_the_words:
        if add_pos_feature:
            tokens = [lemmatizer().lemmatize(token.split('_')[0])+'_'+token.split('_')[1] for token in tokens]
        else:
    ...
    if stemmer_the_words:
        if add_pos_feature:
            tokens = [SnowballStemmer_().stem(token.split('_')[0])+'_'+token.split('_')[1] for token in tokens]
        else:
...
def text_tokens_2(text,lower_bound_percentage=0,higher_bound_percentage=1,minimal_word_length=0,
                   remove_punctuations=False,
                   remove_non_letter_characters=False,
                   lemmatize_the_words=False,
                   stemmer_the_words=False,
...
def Ngrams_for_tokens(Tokens,n=3):
    ...
    for k,v in Ngram_freq.items():
        NgramL.append((k,v))
    NgramFreqTable = pd.DataFrame(NgramL, columns=['Ngram','freq']).sort_values(by='freq', ascending=False)
...
'''
def get_data_from_d2v(train_list_of_list_of_tokens,target_list_of_lists_of_tokens,return_model=False,**kwargs):
    datalist=[]
...
def get_data_from_w2v(train_list_of_list_of_tokens,target_list_of_lists_of_tokens,tfidf_weight=False,normalize_vector_for_TF_IDF=True,return_model=False,**kwargs):
    datalist = []
    ...
    if tfidf_weight:
        ...
        tfidf_vec=get_tfidf_from_context(train_list_of_list_of_tokens,normalize_vector=normalize_vector_for_TF_IDF)
...
'''
def get_data_from_ft_tf_idf(train_list_of_list_of_tokens,target_list_of_lists_of_tokens,tfidf_weight=False,normalize_vector_for_TF_IDF=True,return_model=False,**kwargs):
    datalist = []
    model = FastText(**kwargs)
    model.build_vocab(sentences=train_list_of_list_of_tokens)
    model.train(sentences=train_list_of_list_of_tokens, total_examples=len(train_list_of_list_of_tokens), epochs=kwargs.get('epochs',10))
    Aword=train_list_of_list_of_tokens[0][0]
    dim=len(model.wv.get_vector(Aword))
    if tfidf_weight:
        train_data_dict_for_tf_idf = dict()
...
class gensim_ft:
    ...
    def fit(self,train_list_of_list_of_text,epoch=10):
        ...
        self.model.train(sentences=train_list_of_list_of_tokens, total_examples=len(train_list_of_list_of_tokens), epochs=epoch)
...
def get_key_words_for_dataframe(projectccdf, key_word_method=get_keys_words_use_tf_idf1, remain=.9,
                                projectID_column='ProjectID', StatementOfWork_column='StatementOfWork'):
    """
...
def find_key_words_by_w2v_with_cc_titles(projectccdf,pretrained_map_dict,remain=.9, projectID_column='ProjectID',
                              StatmentOfWork_column='StatementOfWork',cc_title_column='Title'):
    """
...
def get_key_words_for_dataframe_own_tf_idf(projectccdf,remain=.9, projectID_column='ProjectID',
                              StatmentOfWork_column='StatementOfWork',normalize_vector=True):
    """
    ...
    for index,value_dict in enumerate(outputlist):
        id=PIDS[index]
        sorted_tokens=[k for k, v in sorted(value_dict.items(), key=lambda item: item[1], reverse=True)]
